...
public sealed class UpdateJournalEntryCommandValidator : AbstractValidator<UpdateJournalEntryCommand>
{
    ...
    {
        RuleFor(x => x.JournalEntryId).NotEmpty().WithErrorCode("journal_id_required");
        ...
        RuleFor(x => x.Request.RowVersion).NotEmpty().WithErrorCode("row_version_required").Must(IsValidBase64).WithErrorCode("row_version_invalid");
    ...
    private static bool IsValidBase64(string value) { try { Convert.FromBase64String(value); return true; } catch { return false; } }
}
