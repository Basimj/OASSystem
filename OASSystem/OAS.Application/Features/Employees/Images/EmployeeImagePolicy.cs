...
namespace OAS.Application.Features.Employees.Images;

internal static class EmployeeImagePolicy
{
    ...
    public static void EnsureValid(string contentType, byte[] content)
    {
        if (content.Length == 0 || content.Length > MaximumBytes)
            throw new RequestValidationException(new Dictionary<string, string[]> { ["Image"] = ["employee_image_size_invalid"] });
        ...
        if (!valid)
            throw new RequestValidationException(new Dictionary<string, string[]> { ["Image"] = ["employee_image_type_invalid"] });
    }
