...
public sealed class Employee : AuditableEntity<Guid>
{
        ...
        Guid? userAccountId)
    {
        ...
        if (employeeCode.Length > 32)
            throw new DomainException("Employee code cannot exceed 32 characters.");
        ...
        if (UserAccountId.HasValue)
        {
            if (userAccountId != UserAccountId)
                throw new DomainException(
                    "The linked user account cannot be changed or removed.");
    ...
    public void SetPhoto(string? relativePath)
    {
        ...
        if (Photo is not null && Photo.Length > 512)
            throw new DomainException(
                "Employee photo path cannot exceed 512 characters.");
    }
