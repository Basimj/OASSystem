...
public sealed class ReceiptVoucherLine : AuditableEntity<Guid>
{
    ...
    // Legacy accounting-distribution fields are intentionally retained during the
    // Expand phase. AccountId mirrors CounterpartyAccountId for new settlement lines
    // and can be removed only by the later Contract migration.
    ...
    public static ReceiptVoucherLine CreateSettlement(
        Guid id,
        ...
        string? description)
    {
        ValidateIdentity(id, receiptVoucherId, lineNumber);
        ValidateSettlement(
    ...
    // Compatibility factory for pre-multi-currency data/tests. New application flows
    // must use CreateSettlement so that snapshots and server-resolved accounts exist.
    ...
    private static void ValidateIdentity(Guid id, Guid voucherId, int lineNumber)
    {
        ...
        if (voucherId == Guid.Empty) throw new ArgumentException("Receipt voucher id is required.", nameof(voucherId));
        if (lineNumber <= 0) throw new ArgumentOutOfRangeException(nameof(lineNumber));
    }
