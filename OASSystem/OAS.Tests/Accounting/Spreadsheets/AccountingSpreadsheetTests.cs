...
public sealed class AccountingSpreadsheetTests
{
    ...
    [Test] public async Task CustomerAndSupplierImportsGenerateOperationalAndAccountCodes()
    {
        ...
        var customerAccountCode = Repo<Account>().Items.Single(x=>x.Id==customer.AccountId).Code;
        var supplierAccountCode = Repo<Account>().Items.Single(x=>x.Id==supplier.AccountId).Code;
        Assert.That(customerAccountCode, Does.StartWith("110200"));
        Assert.That(customerAccountCode.All(char.IsDigit), Is.True);
        Assert.That(supplierAccountCode, Does.StartWith("210200"));
        Assert.That(supplierAccountCode.All(char.IsDigit), Is.True);
    }
