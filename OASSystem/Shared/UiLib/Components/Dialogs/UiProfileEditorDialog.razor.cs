...
public partial class UiProfileEditorDialog
{
    ...
    {
        _firstName = FirstName; _lastName = LastName; _imageUrl = ImageUrl;
    }

    private Task SelectSectionAsync(string key) { _section = key; _message = null; return Task.CompletedTask; }

    private async Task SaveProfileAsync(MouseEventArgs _)
    {
        if (OnSaveProfile is null || _savingProfile) return;
        _savingProfile = true; _message = null;
        try
        {
            var result = await OnSaveProfile(new UiProfilePersonalData(_firstName, _lastName));
            _message = result.Message; _messageIsError = !result.Succeeded;
        }
    ...
    private async Task ChangePasswordAsync(MouseEventArgs _)
    {
        ...
        _changingPassword = true; _message = null;
        try
        {
