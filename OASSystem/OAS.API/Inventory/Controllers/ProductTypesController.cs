...
public sealed class ProductTypesController(
    ICrudApplicationService<Guid, ProductTypeDto, CreateProductTypeRequest, UpdateProductTypeRequest> service,
    IPermissionChecker permissionChecker) : ControllerBase
{
    [HttpGet]
    public async Task<ActionResult<PagedResult<ProductTypeDto>>> Get(
    ...
    [HttpGet("{id:guid}")]
    public async Task<ActionResult<ProductTypeDto>> GetById(Guid id, CancellationToken cancellationToken)
    {
        await EnsurePermissionAsync(InventoryPermissions.Products.View, cancellationToken);
