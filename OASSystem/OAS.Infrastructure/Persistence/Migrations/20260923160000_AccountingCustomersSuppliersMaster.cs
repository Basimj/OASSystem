...
BEGIN
    ...
    BEGIN
        ...
        VALUES
        (
            @CustomerParentId,@LegacyCustomerControlCode,N'Õ”«» «·⁄„·«¡ «·„—Õ¯·Ì‰',N'Legacy Migrated Customers',
    ...
    SELECT
        ...
        LEFT(COALESCE(n.[NameAr], N'⁄„Ì· „—Õ¯· - ' + CONVERT(nvarchar(36), n.[CustomerId])), 150)
    FROM numbered n;
    ...
    SELECT
        ...
        0,0,0,NULL,0,N' „ ≈‰‘«¡ Â–« «·”Ã·  ·ﬁ«∆Ì« ··Õ›«Ÿ ⁄·Ï „—Ã⁄ „Õ«”»Ì  «—ÌŒÌ √À‰«¡ «· —ÕÌ·.',
        @MigrationNow,@MigrationActor,NULL,NULL,NULL,NULL
    FROM @OrphanCustomers o;
END;
...
BEGIN
    ...
    BEGIN
        ...
        VALUES
        (
            @SupplierParentId,@LegacySupplierControlCode,N'Õ”«» «·„Ê—œÌ‰ «·„—Õ¯·Ì‰',N'Legacy Migrated Suppliers',
    ...
    SELECT
        ...
        LEFT(COALESCE(n.[NameAr], N'„Ê—œ „—Õ¯· - ' + CONVERT(nvarchar(36), n.[SupplierId])), 150)
    FROM numbered n;
    ...
    SELECT
        ...
        0,0,NULL,NULL,0,N' „ ≈‰‘«¡ Â–« «·”Ã·  ·ﬁ«∆Ì« ··Õ›«Ÿ ⁄·Ï „—Ã⁄ „Õ«”»Ì  «—ÌŒÌ √À‰«¡ «· —ÕÌ·.',
        @MigrationNow,@MigrationActor,NULL,NULL,NULL,NULL
    FROM @OrphanSuppliers o;
END;
